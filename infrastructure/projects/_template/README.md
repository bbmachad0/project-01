...
#
# That copies this template, sets the name in project.json, and prints
# next steps.  Or do it manually:
#
#   1. cp -r infrastructure/projects/_template infrastructure/projects/<my_project>
#   2. Edit project.json  - set "name" to the project name (e.g. "sales")
...
# remote state.  Baseline outputs (buckets, databases, IAM) are consumed
# automatically via terraform_remote_state.
#
# Available locals (use in tables.tf / jobs.tf / pipelines.tf):
#   local.config.name                     → this project's name
#   local.baseline.s3_raw_bucket_id       → raw layer bucket
#   local.baseline.s3_refined_bucket_id   → refined layer bucket
#   local.baseline.s3_curated_bucket_id   → curated layer bucket
#   local.baseline.s3_artifacts_bucket_id
#   local.baseline.db_raw_name
#   local.baseline.db_refined_name
#   local.baseline.db_curated_name
#   local.baseline.sfn_execution_role_arn
#   local.baseline.glue_connection_name
#   local.baseline.domain_abbr
#   local.baseline.country_code
