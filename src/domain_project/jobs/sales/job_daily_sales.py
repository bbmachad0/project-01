...
Reads raw sales events from the Iceberg ``raw.sales_events`` table,
computes daily aggregates (revenue, quantity, transaction count), and
writes the result to ``curated.daily_sales``.
...
from domain_project.core.writers import write_iceberg
