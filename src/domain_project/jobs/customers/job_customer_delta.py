...
Reads incremental customer change events from ``raw.customer_events``,
de-duplicates by ``customer_id`` (latest event wins), and merges the
result into the curated ``curated.customers`` Iceberg table using a
...
from domain_project.core.readers import read_iceberg
from domain_project.core.writers import merge_iceberg
