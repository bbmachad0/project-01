...
resource "aws_subnet" "private" {
  ...
  tags = merge(local.common_tags, {
    Name = "${var.project}-subnet-private-${count.index}-${var.env}"
    Tier = "private"
  })
}
...
resource "aws_vpc_endpoint" "s3" {
  ...
  tags = merge(local.common_tags, {
    Name = "${var.project}-vpce-s3-${var.env}"
  })
}
...
resource "aws_vpc_endpoint" "glue" {
  ...
  tags = merge(local.common_tags, {
    Name = "${var.project}-vpce-glue-${var.env}"
  })
}

resource "aws_security_group" "glue_endpoint" {
  name_prefix = "${var.project}-glue-vpce-"
  vpc_id      = aws_vpc.main.id
  ...
  tags = merge(local.common_tags, {
    Name = "${var.project}-sg-glue-vpce-${var.env}"
  })
}
