...
#
#     job_name    = "${local.foundation.domain_abbr}-${local.config.slug}-ingest-events-${var.environment}"
#     description = "Ingest raw events from source into RAW layer."
#     role_arn    = module.iam_glue_job.role_arn
#
#     script_s3_path    = "s3://${local.foundation.s3_artifacts_bucket_id}/jobs/<folder>/job_ingest_events.py"
...
#     connections       = [local.foundation.glue_connection_name]   # VPC isolation
#
#     default_arguments = {
#       "--ENV" = var.environment
#     }
